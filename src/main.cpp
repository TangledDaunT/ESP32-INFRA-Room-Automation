    ...
    if (strcmp(device, "fan") == 0) {
      ...
      if (doc.containsKey("brightness")) {
        ...
        if (doc.containsKey("duration")) {
          ...
          hw_fadeStrip(constrain(val, 0, 255), duration);
        } else {
          hw_setStripBrightness(constrain(val, 0, 255));
        }
        markDirty();
...
void loop() {
  ...
  if (now - lastBlePush >= 2000) {
    if (net_isBleConnected()) {
      net_blePushSensors(sensors_getSmoke(), sensors_getLux(),
                         auto_isPresent());
    }
    lastBlePush = now;
  }
  ...
  if (nvsDirty && (now - lastChangeTime) >= NVS_PERSIST_DEBOUNCE) {
    persistState();
    nvsDirty = false;
    lastNvsWrite = now;
  }
